...
def find_valid_start(log_post_fn, prior, expected_vals, nwalkers, ndim, eps, max_tries=10000, verbose=True):
    ...
    while len(valid) < nwalkers and tries < max_tries:
        ...
        if np.isfinite(lp):
            ...
            if verbose:
                print(f"  Walker {len(valid)}/{nwalkers} trovato al tentativo {tries}: logpost={lp:.2f}")
        tries += 1

    if len(valid) < nwalkers:
        raise RuntimeError(
            f"Solo {len(valid)}/{nwalkers} walker validi dopo {max_tries} tentativi.\n"
            f"Controlla i parametri EFTCamb nel config — il punto fiduciale potrebbe essere instabile."
        )

    print(f"Inizializzazione completata: {nwalkers} walker validi trovati.")
...
def plot_corner(samples, settings, fiducials, myPrior, out_path, nsteps):
    try:
        ...
        myrange = [(samples[:, i].min() - margin[i], samples[:, i].max() + margin[i]) for i in range(samples.shape[1])]

        _ = corner.corner(
            samples,
            labels=settings["params_inference"],
            range=myrange,
            truths=fiducials,
        ...
        plt.savefig(os.path.join(out_path, f'corner_{nsteps}.png'), dpi=150)
        plt.close('all')
        print("Corner ok")
...
main_config = "ModGrav"  # ET_Delta_2CE, ET_2L_2CE, ET_Delta, ET_2L, LVK
...
def Cl_func(universe, params,eftcamb_params, gw_params, dl_GW, bin_edges_dl, z_gal, ll_subset, b_gal, b_GW):
    ...
    _, pkz_camb = universe.camb_Pk(z=zz, k=kk, nonlinear=True, halofit='mead2020')
    bg, kk_eft, zz_eft, pkz_eft = universe.MG_pk(kk, zz, eftcamb_params, True)

    # a z=0 e k intermedia
    iz = np.argmin(np.abs(zz - 0.5))
    ...
    print(f"  @ k=0.01: {np.interp(0.01, kk, ratio_pk):.4f}")
    print(f"  @ k=0.1 : {np.interp(0.1, kk, ratio_pk):.4f}")
    print(f"  @ k=1.0 : {np.interp(1.0, kk, ratio_pk):.4f}")
    print(f"ratio @ k=5.0 : {np.interp(5.0, kk, ratio_pk):.6f}")
    print(f"ratio @ k=10. : {np.interp(9.9, kk, ratio_pk):.6f}")"""
    ...
    def Cl_UPDATE(cosmo_params, eftcamb_params, theta_dict, z_mean_GW, z_mean_gal, b_gal, b_GW,
        ...
        for name in names:
            ...
            if name == 'A_GW' or name == 'gamma_GW':
                ...
                eftcamb_params['RPHmassP0'] = theta_dict['alpha_M']
            elif name == "alpha_B":
                eftcamb_params['RPHbraiding0'] = theta_dict['alpha_B']
            elif name == "wa":
    ...
    def Cl_UPDATE(cosmo_params, eftcamb_params, theta_dict, z_mean_GW, z_mean_gal, b_gal, b_GW, noise_loc_mat_auto,
        ...
        for name in names:
            ...
            if name == 'A_GW' or name == 'gamma_GW':
                ...
                eftcamb_params['RPHbraiding_ODE0'] = theta_dict['alpha_B']
            elif name == "wa":
                eftcamb_params['EFTwa'] = theta_dict['wa']
            elif name == "w0":
                eftcamb_params['EFTw0'] = theta_dict['w0']
        ...
        #print('eftcamb_params post', eftcamb_params)

        #print(f"omega_b calcolato: {params['omega_b']:.6f}")
        #print(f"Omega_b * h^2 atteso: {theta_dict.get('Omega_b', '?') * params['h'] ** 2 if 'Omega_b' in theta_dict else '?'}")
