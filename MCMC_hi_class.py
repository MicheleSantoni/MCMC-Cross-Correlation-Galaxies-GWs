...
import colibri.cosmology_hi_class as cc_MG
...
cosmo_params = configs[cosmo].COSMO_PARAMS_HI
...
main_config = "ModGrav" #DynBack #GW_det  # ET_Delta_2CE, ET_2L_2CE, ET_Delta, ET_2L, LVK
...
z_gal = np.linspace(z_m, z_M, nz_gal.shape[1])  # Redshift grid for galaxy distribution
dl_GW = np.linspace(dlm, dlM, nz_gal.shape[1])  # Luminosity distance grid for gravitational wave sources in Mpc

Lensing = configs[detectors].Lensing

# --- valori di default delle funzioni alpha e dell'espansione ---------------
c_K = 10.0  # alpha_K   (es. 10.0)
c_T = 0.0  # alpha_T   (es. 0.0)
M2_init = 1.0  # es. 1.0
alpha_M = 0.0 # valore fiduciale se non campionato
alpha_B = 0.0
w0 = -1.0
wa = 0.0
Omega_smg_ph = 0.69  # segnaposto: sovrascritto dalla chiusura (Omega_smg=-1)

if Lensing:
    def Cl_func(universe, params, gw_params, dl_GW, bin_edges_dl, z_gal, ll_subset, b_gal, b_GW,
        ...
        kk = np.geomspace(1e-4, 10, 200)  # in [h/Mpc]
        zz = np.linspace(0, z_M, 100)

        # Compute nonlinear matter power spectrum
        bg, kk_out, zz_out, pkz = universe.hi_class_pk(kk, zz, True)  # pkz (Mpc/h)^3

        pkz = np.asarray(pkz, dtype=float)
        if (not np.all(np.isfinite(pkz))) or np.any(pkz <= 0):
        ...
        if bg == 0 and pkz == 0:
            ...
            H_bg = np.asarray(bg["H [1/Mpc]"], dtype=float)  # 1/Mpc
            chi_bg = np.asarray(bg["comov. dist."], dtype=float)  # Mpc
            ...
            alpha_bg = np.asarray(bg["M2_running_smg"], dtype=float)
...
if name_component in ["GW","Gal","Cross"]:
    def Cl_UPDATE(cosmo_params, theta_dict, z_mean_GW, z_mean_gal, b_gal, b_GW,
        ...
        for name in names:
            if name in ('A_GW', 'gamma_GW'):
                b_GW = theta_dict['A_GW'] * (1. + z_mean_GW) ** theta_dict['gamma_GW']
            elif name in ('A_gal', 'gamma_gal'):
                ...
                alpha_M = theta_dict['alpha_M']
            elif name == 'alpha_B':
                alpha_B = theta_dict['alpha_B']
            elif name == 'w0':
                w0 = theta_dict['w0']
            elif name == 'wa':
                wa = theta_dict['wa']
            else:
                params[name] = theta_dict[name]

        # -------------------------
        params['gravity_model'] = 'propto_omega'
        params['parameters_smg'] = f"{c_K}, {alpha_B}, {alpha_M}, {c_T}, {M2_init}"
        params['expansion_model'] = 'wowa'
        params['expansion_smg'] = f"{Omega_smg_ph}, {w0}, {wa}"
        params['Omega_smg'] = -1.0

        universe = cc_MG.cosmo(**params)

        Cl_GG_update, Cl_GWGW_update, Cl_GGW_update = Cl_func(
            universe, params, gw_params, dl_GW, bin_edges_dl,
            z_gal, ll_subset, b_gal, b_GW)
    ...
    def Cl_UPDATE(cosmo_params, theta_dict, z_mean_GW, z_mean_gal, b_gal, b_GW, noise_loc_mat_auto,
...
def log_likelihood(theta,names, verbose=False):
    ...
    if theta_dict['alpha_M'] <0 and  theta_dict['alpha_B'] <0 :  # stability at high z
            return -np.inf

    """if theta_dict['w0'] + theta_dict['wa'] >= 0:  # stability at high z
        return -np.inf"""

    Cl_new = Cl_UPDATE(cosmo_params, theta_dict, z_mean_GW, z_mean_gal, bias_gal, bias_GW, noise_loc_mat_auto,noise_loc_mat,noise_GW, noise_gal, names)  # shape (3, N_ell)
...
with myPool as pool:
    ...
    if not FLAGS.resume:
        ...
        p0 = find_valid_start(
            log_post_fn=sampler.log_prob_fn,
            prior=myPrior,
        ...
        print("Resuming from last point of the chain...")
        old_backend = emcee.backends.HDFBackend(
            os.path.join(FLAGS.fout, "chains.h5"), read_only=True
        )
        p0 = old_backend.get_last_sample()
    ...
    burnin, thin, tau, ESS, ratio = diagnostic(sampler, sampler.iteration, settings)
    samples = summary(sampler, burnin, thin, tau, ESS, ratio, sampler.iteration, settings, FLAGS.fout)
    traceplots(sampler, burnin, settings, FLAGS.fout)
    plot_corner(samples, settings, [settings["fiducial_vals"][p] for p in settings["params_inference"]], myPrior,
                FLAGS.fout, nsteps='')
