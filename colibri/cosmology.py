...
# HI_CLASS
sys.path.insert(0, "/home/cptsu4/santoni/home_cpt/MCMC_template/hi_class/python/build/lib.linux-x86_64-cpython-311")
#PYTHONPATH=/home/cptsu4/santoni/home_cpt/MCMC_template/hi_class/python/build/hi_class/python/build/lib.linux-x86_64-cpython-311

# CLASS
#sys.path.insert(0, "/home/michele-santoni/Home_CPT/MCMC_template/class_public-master/python/build/lib.linux-x86_64-cpython-313")
...
/home/michele-santoni/Home_CPT/COLIBRI_GW/colibri/Extra.py
    ...
    def __init__(self, param_file=None, **kwargs):
        ...
        gravity_model = self.params.get("gravity_model")
        if gravity_model is not None:
            parameters_smg = self.params.get("parameters_smg")
            #print(parameters_smg)
            parameters_smg_list = list(map(float, parameters_smg.split(','))) 	# Convert to list of floats

            alpha_B = parameters_smg_list[1]
            alpha_M = parameters_smg_list[2]
        else:
            alpha_B = 0
            alpha_M = 0


        expansion_model = self.params.get("expansion_model")
        expansion_smg = self.params.get("expansion_smg")
        #print(expansion_smg)

        if expansion_model == "wowa":
            if expansion_smg is None or len(expansion_smg) < 3:
                raise ValueError(
                    "For expansion_model = 'wowa', you must define expansion_smg = Omega_DE, w0, wa with 3 values.")
            expansion_smg_list = list(map(float, expansion_smg.split(',')))  # Convert to list of floats
            #print(expansion_smg_list)


            Omega_DE = expansion_smg_list[0]
            w0 = expansion_smg_list[1]
            wa = expansion_smg_list[2]

            #print("Using WoWA model:")
        ...
        #Omega_m= round(omega_m/(h**2),2)
        #Omega_b= round(omega_b/(h**2),3)
        ...
        #print(Omega_m,Omega_b,self.Omega_nu_tot,self.Omega_ur)
    ...
    def hi_class_pk(self, k, z , nonlinear):
        """
        Compute the matter power spectrum P(k, z) using hi_class,
        and save the background and transfer function in separate files.

        Parameters
        ----------
        params : dict
            CLASS-compatible cosmological parameters.
        k : array-like, optional
            k values in h/Mpc. If None, use CLASS defaults.
        z : array-like, optional
            Redshift values. If None, use CLASS defaults.
        nonlinear : bool
            Whether to compute non-linear power spectrum.

        Returns
        -------
        background: a dictionary with all the info regarding the backgrounds quantities
        k_out : np.ndarray
            Array of k values [h/Mpc]
        z_out : np.ndarray
            Array of redshift values
        pk_out : np.ndarray
            Power spectrum P(k,z) with shape (len(z), len(k)) [Mpc/h]^3
        """
        ...
        try:
            ...
            #print('\t here in the 1st Exception')
            try:
                #print('\t\t trying to clean up the mess')
                cosmo.struct_cleanup()
                cosmo.empty()
            except Exception:
                #print('\t here in the 2st Exception')
                pass
            return 0, 0, 0, 0
        '''
        def _timeout_handler(signum, frame):
            raise RuntimeError("CLASS timeout")
        ...
        try:
            ...
            signal.alarm(20)  # 20s max
            cosmo.compute()
            signal.alarm(0)  # cancel if successful

        except Exception as e:
            signal.alarm(0)
            #print(f'\t here in the Exception: {e}')
            try:
                cosmo.struct_cleanup()
                cosmo.empty()
            except Exception:
        ...
        #print("I'm outside LETSGO \n")
        k = np.atleast_1d(k)
        z_out = np.atleast_1d(z)
        k_out = k.flatten()
        k_Mpc = k_out * self.h

        #print('PK !!')
        # Compute nonlinear power spectrum grid in [Mpc]
        #Pk = _.get_pk_all(kk * h  -> [Mpc], 0.) # get power spectrum: default Mpc^3 units
        # https://github.com/lesgourg/class_public/blob/master/scripts/warmup.py
        pk_nl = cosmo.get_pk_all(k_Mpc, z_out, nonlinear = True, interpolation_kind='cubic')

        # Units conversion to (Mpc/h)^3
        #pk_nl *= self.h ** 3
        #print('BG !!')
        background = cosmo.get_background()
        #print('bg elements', background.keys())

        #print('THE MESS !!')
        cosmo.struct_cleanup()
        cosmo.empty()

        return background, k_Mpc, z_out, pk_nl.T   # Pk shape (len(z), len(k))
