...
os.environ["OMP_NUM_THREADS"] = "10"   # prima di importare camb
# EFTCAMB
sys.path.insert(0, "/home/cptsu4/santoni/home_cpt/MCMC_template/EFTCAMB/")

import camb
print('\nCheck for the CAMB library')
print("USING CAMB FROM:", camb.__file__,"\n")
#print(camb.__doc__)
#print(dir(camb))
...
class cosmo:
    ...
    :param M_ncdm: NCDM masses in eV
    :type M_ncdm: float or list of floats, default = []

    :param Omega_ncdm: NCDM density parameters
    :type Omega_ncdm: float or list of floats, same length as ``M_ncdm``, default = []
    ...
    :param Omega_ncdm_tot: Total WDM density parameter today.
    ...
    :param omega_ncdm: Reduced warm dark matter density parameters, for each species, today.
    :param omega_ncdm_tot: Reduced total warm dark matter density parameters, for each species, today.
    ...
    :param T_ncdm: WDM temperature today, in kelvin.
    ...
    def __init__(self, param_file=None, **kwargs):
        ...
        #print("Loaded parameter keys:", list(self.params.keys()))
        #print("expansion_model value:", self.params.get("expansion_model"))

        # Extract and assign parameters from the dictionary
        h = self.params.get("h") #
        if self.params.get("h") is not None:
            h = self.params.get("h")  #
            self.h = h
            self.H0 = 100. * h
        else:
        ...
        As = self.params.get("A_s") #*1e9

        if self.params.get("Omega_m") is not None:
        ...
        if  self.params.get("Omega_b") is not None:
        ...
        sigma_8 = self.params.get("sigma8") #
        ns = self.params.get("n_s") #

        self.dark_energy_model = self.params.get("dark_energy_model",None)

        Omega_K = self.params.get("Omega_k",0) #
        tau = self.params.get("tau_reio")#
        T_cmb = self.params.get("T_cmb") #
        M_nu = np.atleast_1d(self.params.get("M_nu", []))

        N_ncdm = self.params.get("N_ncdm",0)
        M_ncdm = np.atleast_1d(self.params.get("m_ncdm", []))
        Omega_ncdm =np.atleast_1d(self.params.get("Omega_ncdm", []))
        N_nu = 3 # default

        N_eff = self.params.get("N_ur",3.044) + N_ncdm

        assert np.all(M_nu >= 0.), "All neutrino masses must be larger than or equal to zero."
        assert np.all(M_ncdm > 0.), "All NCDM masses must be larger than zero."
        assert N_eff >= N_nu + N_ncdm, ("Number of effective relativistic species (%.3f) must be >= number of active neutrinos (%i) + WDM species (%i)"% (N_eff, N_nu, N_ncdm))
        assert len(M_nu) <= N_nu, "Too many neutrino masses. Set N_nu >= len(M_nu)"
        assert len(M_ncdm) == len(Omega_ncdm), "Mismatched NCDM masses and densities"

        if As is None and sigma_8 is None:
            raise ValueError("At least one between As and sigma_8 must be specified.")

        self.N_ur = self.params.get("N_ur")
        self.YHe = self.params.get("YHe")

        t15_p4 = 15 / np.pi ** 4
        const_gamma = 1. / const.Msun * 1e3 * const.Mpc_to_m ** 3. / const.rhoch2 / h ** 2.
        ...
        self.K = -Omega_K * (self.H0 / h / const.c) ** 2.
        self.tau = tau
        self.T_cmb = T_cmb
        self.Omega_gamma = const.alpha_BB * T_cmb ** 4. / (const.c * 1e3) ** 2. * const_gamma

        #self.alpha_B = alpha_B
        #self.alpha_M = alpha_M
        self.w0=self.params.get("w0",-1)
        self.wa=self.params.get("wa",0.0)

        self.N_nu = N_nu
        self.M_nu = M_nu
        self.M_nu_tot = np.sum(M_nu)
        self.N_eff = N_eff
        self.Gamma_nu = self.Gamma_nu_inst * (N_eff / N_nu) ** 0.25
        self.T_nu = self.Gamma_nu * T_cmb
        FD_term_nu = self.FermiDirac_integral(M_nu / (const.kB * self.T_nu))
        self.Omega_nu = t15_p4 * self.Omega_gamma * self.Gamma_nu ** 4. * FD_term_nu
        self.Omega_nu_tot = np.sum(self.Omega_nu)

        self.N_ncdm = N_ncdm
        self.M_ncdm = M_ncdm
        self.Omega_ncdm = Omega_ncdm
        self.Gamma_ncdm = self.compute_Gamma_ncdm()
        self.T_ncdm = self.Gamma_ncdm * T_cmb
        self.Omega_ncdm_tot = np.sum(Omega_ncdm)
        self.Delta_N_ncdm_eff = (self.Gamma_ncdm / self.Gamma_nu_inst) ** 4.

        self.massive_nu = len(M_nu[np.where(M_nu > 0)])
        self.massless_nu = N_eff - self.massive_nu - N_ncdm
        ...
        #print("omegas",Omega_m,Omega_b,self.Omega_nu_tot,self.Omega_ur)
        self.Omega_cdm = Omega_m - Omega_b - self.Omega_nu_tot - self.Omega_ur - self.Omega_ncdm_tot
        self.Omega_rad = self.Omega_gamma + self.Omega_ur
        self.Omega_cb = self.Omega_cdm + Omega_b
        self.Omega_cold = self.Omega_cdm + Omega_b + self.Omega_ncdm_tot
        self.Omega_lambda = 1 - self.Omega_cold - self.Omega_nu_tot - Omega_K - self.Omega_rad

        self.omega_K = Omega_K * h ** 2.
        self.omega_cdm = self.Omega_cdm * h ** 2.
        self.omega_ncdm = Omega_ncdm * h ** 2. if len(M_ncdm) > 0 else [0.]
        self.omega_ncdm_tot = self.Omega_ncdm_tot * h ** 2.
        self.omega_cb = self.Omega_cb * h ** 2.
        self.omega_cold = self.Omega_cold * h ** 2.
        self.omega_lambda = self.Omega_lambda * h ** 2.
        self.omega_nu = self.Omega_nu * h ** 2.
        self.omega_nu_tot = self.Omega_nu_tot * h ** 2.
        self.omega_ur = self.Omega_ur * h ** 2.
        self.omega_gamma = self.Omega_gamma * h ** 2.
        self.omega_rad = self.Omega_rad * h ** 2.

        self.f_nu = self.Omega_nu_tot / Omega_m
        self.f_cb = self.Omega_cb / Omega_m
        self.f_cold = self.Omega_cold / Omega_m
        self.f_b = Omega_b / Omega_m
        self.f_c = self.Omega_cdm / Omega_m
        self.f_w = self.Omega_ncdm_tot / Omega_m
        ...
        assert self.Omega_cdm > 1e-10, ("Omega_cdm is too small or even negative, likely because of M_ncdm and T_ncdm values.")

    """ 
    def MG_pk(self, k, z, eftcamb_params, nonlinear=False, debug=True):
        k_out = np.atleast_1d(k).flatten()
        z_out = np.atleast_1d(z).flatten()
        ...
        try:
            ...
            print(eftcamb_params)
            pars.EFTCAMB.initialize_parameters(pars, eftcamb_params, print_header=False)
            pars.EFTCAMB.EFTturnonpiInitial = 1e-3
            pars.EFTCAMB.EFTCAMB_feedback_level = 0

            # ── DIAGNOSTICS ───────────────────────────────────────────────────
            if debug:
                SEP = "=" * 60

                # --- D1: tutti gli attributi esposti dal wrapper Python -------
                print(f"\n{SEP}")
                print("DEBUG D1 — attributi pubblici di pars.EFTCAMB dopo initialize_parameters")
                print(SEP)
                attrs = [a for a in dir(pars.EFTCAMB)
                         if not a.startswith('_')
                         and not callable(getattr(pars.EFTCAMB, a, None))]
                for a in attrs:
                    try:
                        print(f"  {a:45s} = {getattr(pars.EFTCAMB, a)}")
                    except Exception as e:
                        print(f"  {a:45s} = ERROR({e})")

                # Cerca attributi con nomi sospetti (turn, pi, RPH, mass, braid)
                print("\n  → Attributi con 'turn'/'pi'/'initial'/'RPH'/'mass'/'braid' nel nome:")
                for a in dir(pars.EFTCAMB):
                    if any(x in a.lower() for x in
                           ['turn', 'initial', 'rph', 'mass', 'braid', 'kinetic']):
                        try:
                            print(f"     {a} = {getattr(pars.EFTCAMB, a)}")
                        except Exception:
                            print(f"     {a} = <inaccessible>")

                # --- D2: metodi disponibili -----------------------------------
                print(f"\n{SEP}")
                print("DEBUG D2 — metodi pubblici di pars.EFTCAMB")
                print(SEP)
                methods = [a for a in dir(pars.EFTCAMB)
                           if not a.startswith('_')
                           and callable(getattr(pars.EFTCAMB, a, None))]
                for m in methods:
                    print(f"  {m}")

                # --- D3: print_header=True → output Fortran con valori letti -
                print(f"\n{SEP}")
                print("DEBUG D3 — initialize_parameters con print_header=True")
                print("           (mostra i valori che EFTCAMB ha effettivamente letto)")
                print(SEP)
                pars.EFTCAMB.initialize_parameters(pars, eftcamb_params, print_header=True)

                # --- D4: confronto sigma8 GR vs questo run -------------------
                print(f"\n{SEP}")
                print("DEBUG D4 — sigma8: GR baseline vs run corrente")
                print(SEP)
                import copy as _copy

                # GR baseline: EFTflag=0
                pars_gr = camb.CAMBparams()
                pars_gr.WantCls = False;
                pars_gr.WantTransfer = False
                pars_gr.WantScalars = True;
                pars_gr.WantTensors = False
                pars_gr.WantVectors = False;
                pars_gr.DoLensing = False
                pars_gr.Want_CMB = False;
                pars_gr.Want_CMB_lensing = False
                pars_gr.set_cosmology(
                    H0=self.h * 100, ombh2=self.omega_b,
                    omch2=self.omega_cdm, TCMB=self.T_cmb,
                    YHe=self.YHe, nnu=self.N_eff,
                    num_massive_neutrinos=0, mnu=0.0,
                    omk=self.omega_K, tau=self.tau,
                )
                pars_gr.InitPower.set_params(As=self.As, ns=self.ns)
                pars_gr.set_matter_power(redshifts=[0.0], kmax=2.0)
                pars_gr.NonLinear = model.NonLinear_none
                # EFTflag=0 → GR puro
                gr_ep = _copy.deepcopy(eftcamb_params)
                gr_ep['EFTflag'] = 0
                pars_gr.EFTCAMB.initialize_parameters(pars_gr, gr_ep, print_header=False)
                res_gr = camb.get_results(pars_gr)
                s8_gr = res_gr.get_sigma8()[0]
                fs8_gr = res_gr.get_fsigma8()[0]

                # Run corrente
                pars_mg = _copy.deepcopy(pars)
                pars_mg.InitPower.set_params(As=self.As, ns=self.ns)
                pars_mg.set_matter_power(redshifts=[0.0], kmax=2.0)
                pars_mg.NonLinear = model.NonLinear_none
                res_mg = camb.get_results(pars_mg)
                s8_mg = res_mg.get_sigma8()[0]
                fs8_mg = res_mg.get_fsigma8()[0]

                print(f"  GR (EFTflag=0):   sigma8={s8_gr:.6f}   f*sigma8={fs8_gr:.6f}")
                print(f"  MG (run corrente): sigma8={s8_mg:.6f}   f*sigma8={fs8_mg:.6f}")
                delta = abs(s8_mg - s8_gr)
                if delta < 1e-5:
                    print("  ❌ sigma8 IDENTICO → EFTCAMB non sta modificando le perturbazioni.")
                    print("     Cause più probabili (in ordine):")
                    print("     1. I nomi delle chiavi RPH nel dict non vengono riconosciuti")
                    print("        → guarda D3: se c_M=0.000 anche con c_M=0.5 nel dict, è questo.")
                    print("     2. EFTturnonpiInitial non raggiunge il Fortran (attributo Python fittizio)")
                    print("        → cerca 'EFTturnonpiInitial' nell'output D1.")
                    print("     3. Ghost/gradient instability check silenzia le perturbazioni")
                    print("        → riprova con EFTCAMB_feedback_level=3 e cerca 'EFT check failed'.")
                else:
                    print(f"  ✓ sigma8 devia di {delta:.6f} → EFTCAMB modifica le perturbazioni.")

                # --- D5: varianti nomi chiavi RPH ----------------------------
                print(f"\n{SEP}")
                print("DEBUG D5 — test varianti nomi chiavi RPH nel dict (c_M=0.5)")
                print(SEP)
                name_variants = [
                    ('RPHmassPparams', 'RPHbraidingparams'),
                    ('RPHMassPparams', 'RPHBraidingparams'),
                    ('RPH_mass_P_params', 'RPH_braiding_params'),
                    ('RPHmassPAmplitude', 'RPHbraidingAmplitude'),
                    ('RPH_cM', 'RPH_cB'),
                ]
                for (k_m, k_b) in name_variants:
                    ep_v = _copy.deepcopy(eftcamb_params)
                    ep_v[k_m] = [0.5]
                    ep_v[k_b] = [0.5]
                    ep_v['EFTturnonpiInitial'] = 1e-3
                    pars_v = camb.CAMBparams()
                    pars_v.WantCls = False;
                    pars_v.WantTransfer = False
                    pars_v.WantScalars = True;
                    pars_v.WantTensors = False
                    pars_v.WantVectors = False;
                    pars_v.DoLensing = False
                    pars_v.Want_CMB = False;
                    pars_v.Want_CMB_lensing = False
                    pars_v.set_cosmology(
                        H0=self.h * 100, ombh2=self.omega_b,
                        omch2=self.omega_cdm, TCMB=self.T_cmb,
                        YHe=self.YHe, nnu=self.N_eff,
                        num_massive_neutrinos=0, mnu=0.0,
                    ...
                    pars_v.InitPower.set_params(As=self.As, ns=self.ns)
                    pars_v.set_matter_power(redshifts=[0.0], kmax=2.0)
                    pars_v.NonLinear = model.NonLinear_none
                    try:
                        pars_v.EFTCAMB.initialize_parameters(pars_v, ep_v, print_header=False)
                        res_v = camb.get_results(pars_v)
                        s8_v = res_v.get_sigma8()[0]
                        marker = "✓ CAMBIA!" if abs(s8_v - s8_gr) > 1e-5 else "= GR"
                        print(f"  {k_m:30s}  sigma8={s8_v:.6f}  {marker}")
                    except Exception as e:
                        print(f"  {k_m:30s}  ERRORE: {e}")
            ...
            logint = 100
            n_points = int(logint * np.log10(np.max(k_out) / np.min(k_out)))
            dlogk = 2. * np.log10(np.max(k_out) / np.min(k_out)) / n_points
            k_max = float(np.max(k_out)) * 10 ** dlogk

            pars.set_matter_power(
                redshifts=np.sort(z_out)[::-1].tolist(),
                kmax=k_max,
            )
            if nonlinear:
                pars.NonLinear = model.NonLinear_both
                pars.halofit_version = 9
            else:
            ...
            kh, _, pk = results.get_matter_power_spectrum(
                minkh=float(np.min(k_out)),
                maxkh=float(np.max(k_out)),
                npoints=n_points,
            )

            a_out = 1.0 / (1.0 + z_out)
            bg_dens = results.get_background_densities(a=a_out)
            Omega_m_z = (bg_dens['cdm'] + bg_dens['baryon']) / bg_dens['tot']
            ...
            mask = (f_z > 0) & (Omega_m_z > 0) & (Omega_m_z < 1)
            gamma_z = np.where(mask, np.log(f_z) / np.log(Omega_m_z), np.nan)

            try:
                c_M = eftcamb_params['RPHmassPparams'][0]
                c_B = eftcamb_params['RPHbraidingparams'][0]
                Omega_DE = bg_dens['de'] / bg_dens['tot']

                print("alphas M, B and omega_DE", c_M, c_B, Omega_DE)

                background = {
                    'z': z_out,
                    'H': results.hubble_parameter(z_out) / 2.99792458e5,
                    'chi': results.comoving_radial_distance(z_out),
                    'alpha_M': c_M * Omega_DE,
                    'Omega_m_z': Omega_m_z,
                    'gamma_z': gamma_z,
                }
                ...
                background = {
                    ...
                    'H': results.hubble_parameter(z_out) / 2.99792458e5,
                    'chi': results.comoving_radial_distance(z_out),
                    'alpha_M': np.zeros_like(z_out),
                    'Omega_m_z': Omega_m_z,
                    'gamma_z': gamma_z,
                }

            return background, kh, z_out, pk

        except Exception as e:
            print(f"[MG_pk] EFTCAMB failed: {e}")
            return 0, 0, 0, 0"""


    def MG_pk(self, k, z,eftcamb_params, nonlinear=False,debug=False):
        """
        Compute the matter power spectrum P(k, z) using EFTCAMB.
        ...
        z : array-like
            Redshift values.
        nonlinear : bool
            Whether to compute non-linear power spectrum (halofit).
            WARNING: halofit not calibrated for MG — use with caution.

        Returns
        -------
        background : dict
            Background quantities (H, comoving distance, ...).
        k_out : np.ndarray
            k values in h/Mpc.
        z_out : np.ndarray
            Redshift values.
        pk_out : np.ndarray
            Power spectrum P(k,z), shape (len(z), len(k)) [(Mpc/h)^3].
        """
        ...
        try:
            ...
            # Unifica cosmo + EFTCAMB in un unico set_params — come il notebook
            camb_dict = {
                'H0': self.h * 100,
                'ombh2': self.omega_b,
                'omch2': self.omega_cdm,
            ...
            camb_dict.update(eftcamb_params)  # merge EFTCAMB params

            pars = camb.set_params(**camb_dict)

            if self.dark_energy_model == 'ppf' or self.dark_energy_model == 'fluid' :
                print("I am here", self.dark_energy_model)
            ...
            #default hubble_units=True, k_hunit=True
            pars.set_matter_power(
                redshifts=np.sort(z_out)[::-1].tolist(),
                kmax=k_max,
                #k_per_logint=5,
            ...
            # CHECK
            #print("w0 :", pars.DarkEnergy.w)
            #print("wa :", pars.DarkEnergy.wa)
            ...
            #   EFTw0 = -0.900
            #   EFTwa =  0.300
            # CHECK 2 — w(a) effettivo tramite get_eft_functions
            #a_arr = np.linspace(0.1, 1.0, 50)
            #eft_funcs = pars.EFTCAMB.get_eft_functions(pars, a_arr)
            #print("chiavi disponibili:", eft_funcs.keys())
            # cerca 'wDE' o 'w' — deve seguire w0 + wa*(1-a)
            #if 'wDE' in eft_funcs:
            #    w_a = eft_funcs['wDE']
            #    print(f"w(a=1.0) = {np.interp(1.0, a_arr, w_a):.4f}  (atteso: w0={EFTCAMB_PARAMS['EFTw0']})")
            #    print(f"w(a=0.5) = {np.interp(0.5, a_arr, w_a):.4f}  (atteso: {EFTCAMB_PARAMS['EFTw0'] + EFTCAMB_PARAMS['EFTwa'] * 0.5:.4f})")
            ...
            f_z = (fs8 / s8)[::-1]  # same order of z_out
            s8_ordered = s8[::-1]
            ...
            try:
                c_M = eftcamb_params.get('RPHalphaM_ODE0', 0.0)
                #c_B = eftcamb_params.get('RPHbraiding_ODE0', 0.0)
                Omega_DE = bg_dens['de'] / bg_dens['tot']
                #print("alphas M, B",c_M, c_B)
                background = {
                    'z': z_out,
                    'H': results.hubble_parameter(z_out) / 2.99792458e5,  # 1/Mpc
                    'chi': results.comoving_radial_distance(z_out),  # Mpc
                    'alpha_M': c_M * Omega_DE,
                    'Omega_m_z': Omega_m_z,
                    'gamma_z': gamma_z
                }
                ...
                #print(f"Warning: alpha_M non calcolabile ({e}), impostato a zero.")
                background = {
                    'z': z_out,
                    'H': results.hubble_parameter(z_out) / 2.99792458e5,  # 1/Mpc
                    'chi': results.comoving_radial_distance(z_out),  # Mpc
    ...
    # -------------------------------------------------------------------------------
    def check_stability(self, params,theta):
        """
        Return 1 if CLASS runs without error for these parameters,
        otherwise return 0.
        """
        #print('\nUpdating parameters...')
    ...
    def compute_Gamma_ncdm(self):
        ...
        return GN*(self.Omega_ncdm*self.h**2.*den/self.M_ncdm)**(1./3.)
    ...
    def Omega_ncdm_z(self, z):
        ...
        :return: array of shape ``(self.N_ncdm, len(z))`` containing :math:`\Omega_\\nu(z)`.
        
        """
        MM,ZZ = np.meshgrid(np.atleast_1d(self.M_ncdm),np.atleast_1d(z),indexing='ij')
        Y     = MM/((1.+ZZ)*const.kB*np.expand_dims(self.T_ncdm,1))
        F     = self.FermiDirac_integral(Y)
        GW    = np.expand_dims(self.Gamma_ncdm**4.,1)
    ...
    def H(self, z):
        ...
        # WDM contribution
        yw               = np.outer(self.M_ncdm/(const.kB*self.T_ncdm), 1./(1.+z))
        Fw               = self.FermiDirac_integral(np.array(yw))
        ncdm_contribution = prefactor*np.expand_dims(self.Gamma_ncdm**4.,1)*Fw
        # H(z)
        return self.H0*(self.Omega_cdm   *(1+z)**3 +
                        ...
                        ur_contribution +
                        np.sum(ncdm_contribution,axis=0) +
    ...
    # WDM suppression
    #-------------------------------------------------------------------------------
    def WDM_suppression(self, k, z, M_ncdm, nonlinear = False):
        """
        ...
        :param M_ncdm: Mass of the warm dark matter particle in keV.
        :type M_ncdm: float
        ...
        K,Z = np.meshgrid(k,z)
        if not nonlinear:
            alpha_linear = 0.049*M_ncdm**(-1.11)*(self.Omega_cdm/0.25)**0.11*(self.h/0.7)**1.22 # Mpc/h
            nu           = 1.12
            return (1.+(alpha_linear*K)**(2.*nu))**(-10./nu)

        else:
            nu, l, s = 3., 0.6, 0.4
            alpha    = 0.0476*(1./M_ncdm)**1.85*((1.+Z)/2.)**1.3 # Mpc/h
    ...
    def camb_Pk(self,
        ...
        cambparams = {
            ...
            'ombh2': self.omega_b,
            'omch2': self.omega_cdm + self.omega_ncdm_tot,
            'omk': self.Omega_K,
            'H0': 100. * self.h,
            'As': self.As,
            'ns': self.ns,
        ...
        #print('\nquesti sono i parametri di CAMB: \n', cambparams)
        # kwargs
    ...
    def class_Pk(self,
        ...
        if self.N_ncdm > 0 and self.massive_nu > 0.:
            params['m_ncdm'] += ', ';
            params['T_ncdm'] += ', '
            params['m_ncdm'] += ', '.join(str(x) for x in self.M_ncdm)
            params['T_ncdm'] += ', '.join(str(x) for x in self.Gamma_ncdm)
        elif self.N_ncdm > 0:
            params['m_ncdm'] = ', '.join(str(x) for x in self.M_ncdm)
            params['T_ncdm'] = ', '.join(str(x) for x in self.Gamma_ncdm)
        ...
        for c1 in var_1:
            for c2 in var_2:
                ...
                for ind_z in range(nz):
                    ...
                    TF['d_ncdm'] = np.zeros_like(TF['k (h/Mpc)'])
                    for inw in range(self.N_ncdm):
                        index = inw + self.massive_nu
                        TF['d_ncdm'] += self.Omega_ncdm[inw] / self.Omega_ncdm_tot * TF['d_ncdm[%i]' % index]
                    TF['d_cold'] = (self.Omega_cdm * TF['d_cdm'] +
                                    self.Omega_ncdm_tot * TF['d_ncdm'] +
    ...
    def EisensteinHu_Pk(self,
        ...
        if np.sum(self.M_nu) != 0.:
            warnings.warn("Eisenstein Hu_Pk is not able to reproduce massive neutrinos as it uses the Eisenstein & Hu approximation (1998) for the linear power spectrum. The Omega_nu parameter will be transferred to Omega_lambda such that Omega_lambda -> (Omega_lambda + Omega_nu)")
