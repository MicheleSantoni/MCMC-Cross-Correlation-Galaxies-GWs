...
class limber():
    ...
    def load_power_spectra(self, k, z, power_spectra):
        ...
        `power_spectra` and `galaxy_bias` must be a 2D array of shape ``(len(z), len(k))`` which contains the power spectrum (in units of :math:`(\mathrm{Mpc}/h)^3`)
        and galaxy bias evaluated at the scales and redshifts specified above.
    ...
    def load_shear_window_functions(self, z, nz, name = 'shear'):
        ...
        Given a galaxy distribution in a redshift bin :math:`n^{(i)}(z)`, the equation is:
    ...
    def limber_angular_power_spectra(self, l, windows = None):
        ...
        HH       = self.Hubble
        cH_chi2  = self.c_over_H_over_chi_squared   ######################################  QUI
        Cl       = {}
