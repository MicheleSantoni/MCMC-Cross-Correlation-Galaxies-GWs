...
class limber():
    ...
    def __init__(self, cosmology, z_limits=(0., 5.)):
        ...
        # Factor c/H(z)/f_K(z)^2  [Mpc/1]*[h/Mpc]**2 =[h^2/Mpc]
    ...
    def load_power_spectra(self, k, z, power_spectra):
        ...
        interp_func = RegularGridInterpolator((self.z, self.k),power_spectra,kind_of_interpolation,bounds_error=False,fill_value=0.)
    ...
    #-----------------------------------------------------------------------------------------
    def load_gw_lensing_window_functions(self,chi_interp,alpha_M_interp, z, n_dl, H_0, Omega_m, ll, name = 'lensing_GW'):
        def dL_from_C(chi_interp, z, alpha_interp):
            """
            ...
            chi = np.asarray(chi_interp(z))  # Mpc

            """chi = np.asarray(C.comoving_distance(z)) / C.h  # DA COLIBRI.COSMOLOGY
            diff_abs = np.abs(chi_test - chi_Mpc)
            mask = chi_Mpc > 0
            diff_rel = np.where(mask, diff_abs / chi_Mpc, np.nan)

            print(f"max |Δχ|        : {diff_abs.max():.3e} Mpc")
            print(f"max |Δχ/χ|      : {np.nanmax(diff_rel) * 100:.4f} %")
            print(f"rms |Δχ/χ|      : {np.sqrt(np.nanmean(diff_rel ** 2)) * 100:.4f} %")"""

            exp = exp_factor_alphaM(z, alpha_interp)
            return (1.0 + z) * chi * exp
        ...
        norm_const = simpson(n_dl , x=dL_from_C(chi_interp, z, alpha_M_interp), axis=1)
    ...
    def load_galaxy_lensing_window_functions(self, z, n_z, H_0, Omega_m, ll, name = 'galaxy_lensing'):
        ...
        self.window_function[name]=np.tile(np.array(self.window_function[name]).reshape(-1,1), (1,len(ll))) #[1/Mpc]
    ...
    # -----------------------------------------------------------------------------------------
    def load_gravitational_wave_window_functions(self,H_interp,chi_interp, alpha_M_interp, z, n_dl, ll, bias=1.0, name='GW'):
        """
        ...
        # n_dl [1/Gpc]  ----> check
        norm = simpson(n_dl, x= dL_from_C(chi_interp, z, alpha_M_interp), axis=1) #
        ...
        for i_bin in xrange(n_bins):
            ...
            #        =  [1]*     [1/Gpc]        * [Mpc/1]    * [1/Mpc] / [Mpc/Gpc]
            w_vals = bias[i_bin] * ndl_on_z * jac_dL_bg * H_bg_on_z / norm[i_bin] # [1/Mpc]
            windows.append(si.interp1d(z_int, w_vals, kind='cubic', bounds_error=False, fill_value=0.0))
    ...
    # RSD WINDOW FUNCTION
    #-----------------------------------------------------------------------------------------
    def load_rsd_window_functions(self,H_interp,Omz_interp, z, n_z, ll, name = 'RSD'):
        ...
        self.window_function[name]=np.array(self.window_function[name]) #[1/Mpc]
            
    #-----------------------------------------------------------------------------------------
    # LSD WINDOW FUNCTION
    #-----------------------------------------------------------------------------------------
    def load_lsd_window_functions(self,H_interp,chi_interp,alpha_M_interp,Omz_interp , z, n_dl, ll, name = 'LSD'):
        def dL_from_C(chi_interp, z, alpha_interp):
            """
            Luminosity distance d_L(z) in Mpc from your colibri cosmology `C`.
            Assumes C.comoving_distance(z) returns comoving distance in Mpc/h.
        ...
        conf_H = H_interp(z) / (1.0 + z)  # ≡ H(z)/(1+z) H(z) in 1/Mpc
        r_conf_H = chi_interp(z) * conf_H  # ≡ χ(z) * H(z)/(1+z)  Adimensional
        gamma = r_conf_H / (1.0 + r_conf_H)

        # bg branch: H and r from bg
        r_z = chi_interp(z)  # [Mpc]
        alpha_M_z = alpha_M_interp(z)
        exp_val = exp_factor_alphaM(z, alpha_M_interp)

        jj = ((1 + z) *1 / H_z + r_z + r_z  * 0.5 * alpha_M_z) * exp_val  # [Mpc]
    ...
    def limber_angular_power_spectra(self,H_interp,chi_interp,h, l, windows = None):
        ...
        def geometric_factor_bg(chi_interp, z, h, K_h=0.0, z0=0.0):
            ...
            """
            chi_Mpc = chi_interp(z) - chi_interp(z0)  # Mpc/1
            #chi_h = h * chi_Mpc  # Mpc/1

            if np.allclose(K_h, 0.0):
                return  chi_Mpc #Mpc        old: chi_h  flat: f_K(chi)=chi  [Mpc/h]

            if np.any(K_h > 0):
                # closed: f_K = sin(sqrt(K)*chi)/sqrt(K)
                return np.sin(np.sqrt(K_h) * chi_Mpc) / np.sqrt(K_h)  # Mpc/h
            else:
                # open:   f_K = sinh(sqrt(|K|)*chi)/sqrt(|K|)
                Kabs = np.abs(K_h)
                return np.sinh(np.sqrt(Kabs) * chi_Mpc) / np.sqrt(Kabs)  # Mpc/h
        ...
        """geom_old = np.asarray(self.geometric_factor)  # Mpc/h  (da colibri)
        geom_new = geometric_factor_bg(chi_interp, zz, h) * h  # geom_int * h  → Mpc/h

        ratio = geom_new / geom_old
        print("=" * 55)
        print("  geom_new / geom_old  (entrambi in Mpc/h)")
        print("=" * 55)
        print(f"  min   : {ratio.min():.6f}")
        print(f"  max   : {ratio.max():.6f}")
        print(f"  medio : {ratio.mean():.6f}")
        print(f"  std   : {ratio.std():.6f}")
        print(f"\n  z=0.5 : {np.interp(0.5, zz, ratio):.6f}")
        print(f"  z=1.0 : {np.interp(1.0, zz, ratio):.6f}")
        print(f"  z=2.0 : {np.interp(2.0, zz, ratio):.6f}")
        print(f"  z=5.0 : {np.interp(5.0, zz, ratio):.6f}")"""

        # set the right range
        geom_int = geometric_factor_bg(chi_interp, zz, h)  # shape: (n_z,)  in Mpc/1
        geom_h = geom_int * h  # Mpc → Mpc/h

        #H_inverse = (1 / H_interp(zz)) * (1 / (geom_int ** 2)) #[Mpc/1]*[1^2/Mpc^2] = [1/Mpc]
        H_inv_h = (h / H_interp(zz))*(1/ geom_h ** 2 ) # [Mpc/h]*[h^2/Mpc^2] h/Mpc
        ...
        for il in xrange(n_l):
            for iz in xrange(n_z):
                ...
                k_h = (l[il] + 0.5) / geom_h[iz]  # h/Mpc  ✓
                PS_lz[il, iz] = np.asarray(power_spectra([(zz[iz],k_h)])).item() #PS_lz in (Mpc/h)^3
        #end = time.time()
        ...
        W = {
            key: np.array([
                [windows_to_use[key][i, j](zz) for j in range(n_l)]
                for i in range(n_bins[idx])
            ]).transpose(0, 2, 1)  # (n_bins, n_l, n_z) → (n_bins, n_z, n_l)
            for idx, key in enumerate(keys)
        } """
        #print('keys',keys)
        #print('W',W)

        """np.save('debug_H_inv_h.npy', H_inv_h)  # shape (n_z,)
        np.save('debug_PS_lz.npy', PS_lz)  # shape (n_l, n_z)

        # W è un dict key → array (n_bins, n_l, n_z)
        for key, arr in W.items():
            np.save(f'debug_W_{key}.npy', arr)
        """

        """
        print("Salvati: H_inv_h, PS_lz, W per ogni key")
        print(f"  H_inv_h shape : {H_inv_h.shape}")
        print(f"  PS_lz shape   : {PS_lz.shape}")
        for key, arr in W.items():
            print(f"  W[{key}] shape : {arr.shape}")
        """
                ...
                if key_X == key_Y:
                    for bin_i in range(n_bins[index_X]):
                        for bin_j in range(bin_i, n_bins[index_Y]):
                            # [1/Mpc] *  [1/Mpc] * [1/Mpc] * (Mpc/h)^3
                            val = simpson(
                                H_inv_h * W_X[bin_i] * W_Y[bin_j] * PS_lz,
                                x=zz
                            )
    ...
    #-----------------------------------------------------------------------------------------   
    def limber_angular_power_spectra_lensing_auto(self,H_interp,chi_interp, alpha_M_interp, l,  s_gal, beta, windows = None, n_points=20, n_points_x=20, grid_x='mix', n_low=5, n_high=5, Delta_z=0.05, z_min=1e-05):

        def geometric_factor_bg(chi_interp, z, K_h=0.0, z0=0.0):
            """
            Return f_K(chi[z]-chi[z0]) in units of Mpc/h.
            bg['comov. dist.'] is assumed in Mpc (flat comoving distance).
            K_h must be in (h/Mpc)^2.
        ...
        # Check convergence with (l, k, z):
        assert np.atleast_1d(l).min() > self.k_min  * geometric_factor_bg(chi_interp, self.z_min), "Minimum 'l' is too low. Extend power spectra to lower k_min? Use lower z_min for power spectrum?"
        assert np.atleast_1d(l).max() < self.k_max  * geometric_factor_bg(chi_interp, self.z_max), "Maximum 'l' is too high. Extend power spectra to higher k_max? Use higher z_max for power spectrum?"
        ...
        #print('z',z_bg)
        #print('H',H_interp)
        #print('comoving distance',chi_interp)
        #print('alpha_M',alpha_M_interp)

        # JJ is here 1D
        def JJ(tracer, x, H_interp, chi_interp, alpha_M_interp, exp_interp_f):
            if 'gal' in tracer:
                return np.ones_like(x)
            H_x = H_interp(x)
            r_x = chi_interp(x)
            alphaM = alpha_M_interp(x)
            exp_val = exp_interp_f(x)
            return ((1 + x) / H_x + r_x + r_x * 0.5 * alphaM) * exp_val
        ...
        # DOPO: integra cumulativamente su griglia
        #print("self.z_max",self.z_max)
        z_grid = np.linspace(0, max(self.z_max, 10)  + 0.01, 1000)
        integrand_grid = 0.5 * alpha_M_interp(z_grid) / (1 + z_grid)
        cumint = np.concatenate([[0], np.cumsum(np.diff(z_grid) *
                                                (integrand_grid[:-1] + integrand_grid[1:]) / 2)])
        exp_interp = si.interp1d(z_grid, np.exp(cumint), kind='cubic', bounds_error=False,
                                 fill_value=(np.exp(cumint[0]), np.exp(cumint[-1])))

        def lensing_int(H_interp,chi_interp,alpha_M_interp,exp_interp,z1, z2, ll, bin_i, bin_j, WX_, WY_, tX, tY,b1,b2, n_points=30, p=1, z_min=1e-04, grid='mix', show_plot=False):
            ...
            # Old way: slow
            """
            PS_ = np.squeeze(np.asarray([[[[power_spectra((z_val, k_val)) for k_val, z_val in
                                            zip((ll[l] + 0.5) / r_x[:, k, i], x[:, k, i])]
                                           for i in range(len(z1))]
                                          for k in range(len(z2))]
                                         for l in range(len(ll))]))
            """

            k_all = (ll[:, None, None, None] + 0.5) / r_x[None, :, :, :]  # (n_l, n_pts_x, n_z2, n_z1)
            z_all = np.broadcast_to(x[None, :, :, :], k_all.shape)
            pts = np.stack([z_all.ravel(), k_all.ravel()], axis=-1)
            PS_ = power_spectra(pts).reshape(k_all.shape)  # (n_l, n_pts_x, n_z2, n_z1)
            PS_ = PS_.transpose(0, 2, 3, 1)  # (n_l, n_z1, n_z2, n_pts_x)
            ...
            I1_ = trapezoid(my_int, x=x.transpose(1, 2, 0)[None, ...], axis=-1)
            ...
            I2_ = np.asarray( [[ trapezoid( WY_[:, i1] * JJ(tY, z2[:, i1],H_interp, chi_interp, alpha_M_interp,exp_interp)*I1_[l, :, i1]/r_2[:, i1], x=z2[:, i1], axis=0 ) for i1 in range(len(z1))] for l in range(len(ll))])

            I3_ =  trapezoid( WX_ * JJ(tX, z1,H_interp, chi_interp, alpha_M_interp,exp_interp)*I2_/r_1, x=z1, axis=1 )
                ...
                for bin_i in xrange(n_bins[index_X]):
                    ...
                    for bin_j in my_range:
                        if  (bin_j==bin_i): #and (key_Y==key_X):                      and ('gal' in key_Y) and (bin_j<=2)
                            ...
                            Cl['%s-%s' %(key_X,key_Y)][bin_i,bin_j,:] = l**2*(l+1)**2/((l+0.5)**4)*lensing_int(H_interp,chi_interp,alpha_M_interp,exp_interp ,zzs[bin_i], z2s_, l, bin_i, bin_j, WX, WY, key_X, key_Y,b1,b2,
    ...
    def limber_angular_power_spectra_lensing_cross(
            self,H_interp,chi_interp, alpha_M_interp, l, s_gal, beta, windows=None,
        ...
        def JJ(tracer, x,H_interp, chi_interp, alpha_M_interp,exp_interp):
            if 'gal' in tracer:
                return np.ones_like(x)
            H_x = H_interp(x)
            r_x = chi_interp(x)
            alphaM = alpha_M_interp(x)
            exp_val = exp_interp(x)
            return ((1 + x) / H_x + r_x + r_x * 0.5 * alphaM) * exp_val
        ...
        def A_L(chi, z, tracer, r_z1, bs, H_vals):
            if 'gal' in tracer:
                ...
                conf_H = H_vals / (1 + z) #1/Mpc
                return 0.5 * (((r_z1 - chi) / chi) * (bs - 2) + (1 / (1 + r_z1 * conf_H))) #Adim

        power_spectra = self.power_spectra_interpolator  # expects points=(z,k)

        # Fuori da lensing_int, una volta sola
        z_grid = np.linspace(0, max(self.z_max, 10) + 0.01, 1000)
        integrand_grid = 0.5 * alpha_M_interp(z_grid) / (1 + z_grid)
        cumint = np.concatenate([[0], np.cumsum(np.diff(z_grid) *
                                                (integrand_grid[:-1] + integrand_grid[1:]) / 2)])
        exp_interp = si.interp1d(z_grid, np.exp(cumint), kind='cubic',
                                 bounds_error=False, fill_value='extrapolate')

        # ---------------- core integrand ----------------
        def lensing_int(H_interp, chi_interp,alpha_M_interp,exp_interp, z1, z2, ll, bin_i, bin_j, WX_, WY_, tX, tY, n_points=30, p=1, z_min=1e-04, grid='mix', show_plot=False):

            r_1 = chi_interp(z1)  # (Nz1,) Mpc
            z2 = np.asarray(z2)  # (Nz2, Nz1)
            r_2 = chi_interp(z2)  # (Nz2, Nz1) Mpc
            ...
            t1 = np.transpose(
                ...
                * (1.0 + z2)
                * JJ(tY, z2, H_interp, chi_interp, alpha_M_interp,exp_interp),
                (1, 0)
            )
            ...
            PS_ = np.squeeze(np.asarray([[[
                power_spectra((z_val, k_val)) for k_val, z_val in
                zip((ll[l] + 0.5) / r_2[:, i], z2[:, i])  # r2/z2 are 2D
            ] for i in range(len(z1))] for l in range(len(ll))]))
           """
            # (z,k) fast way
            ll_arr = np.atleast_1d(ll)  # (n_l,)
            k_all = (ll_arr[:, None, None] + 0.5) / r_2[None, :, :]  # (n_l, Npts, Nz1)  ← r_2 è (Npts, Nz1)
            z_all = np.broadcast_to(z2[None, :, :], k_all.shape)  # (n_l, Npts, Nz1)
            pts = np.stack([z_all.ravel(), k_all.ravel()], axis=-1)  # (n_l*Npts*Nz1, 2)
            PS_ = power_spectra(pts).reshape(k_all.shape)  # (n_l, Npts, Nz1)
            PS_ = PS_.transpose(0, 2, 1)  # (n_l, Nz1, Npts)
            ...
            # ∫ dz2
            I1_ = np.asarray([
                [trapezoid(my_int[lidx, i1], x=z2[:, i1], axis=0)  #  scalar for (lidx,i1)
                 for i1 in range(len(z1))]
                for lidx in range(len(ll))
            ])  # (n_l, L)

            # ∫ dz1
            I2_ = trapezoid(WX_ * I1_ / r_1 * (1 / H_interp(z1)), x=z1, axis=1)  # (n_l,)
        ...
        for index_X in range(nkeys):
            ...
            # Windows for X — calcolato una volta per index_X
            W_X = np.array([
            ...
            pref = l_used * (l_used + 1) / ((l_used + 0.5) ** 2)  # (n_l,)

            n_diag_max = n_bins[index_X]  # upper bound, then clipped for every index_Y

            z2s_cache = {
                bin_i: np.linspace(z_min, zzs[bin_i], n_points)
                for bin_i in range(n_diag_max)
            }  # {bin_i: (Npts, L)}

            W_Y_cache = {
                key_Y: {
                    bin_i: np.array([
                        [windows_to_use[key_Y][iy, j](z2s_cache[bin_i]) for j in range(n_l)]
                        for iy in range(n_bins[idx_Y])
                    ])  # (n_bins_Y, n_l, Npts, L)
                    for bin_i in range(n_diag_max)
                }
                for idx_Y, key_Y in enumerate(keys)
            }

            for index_Y in range(nkeys):
                key_Y = keys[index_Y]
                n_diag = min(n_bins[index_X], n_bins[index_Y])

                for bin_i in range(n_diag):
                    bin_j = bin_i

                    z2s_ = z2s_cache[bin_i]
                    WY = W_Y_cache[key_Y][bin_i][bin_j]  # (n_l, Npts, L)
                    WX = W_X[bin_i]  # (n_l, L)

                    I_vec = lensing_int(
                        H_interp, chi_interp, alpha_M_interp,exp_interp,
                        zzs[bin_i], z2s_, l,
                        bin_i, bin_j, WX, WY,
                        key_X, key_Y,
                        z_min=z_min, n_points=n_points_x, grid=grid_x
    ...
    def load_gravitational_wave_window_functions_old(self, z, ndl, H0, omega_m, omega_b, ll, bias=1.0, name='GW'):
        ...
        jac_dl = ((1 + self.z_integration) * ((const.c / self.Hubble)/h) + (conversion.luminosity_distance(self.z_integration).value) / (1 + self.z_integration))  # [Mpc]
        # print('\n GW OLD:')
        ...
        """print(f"self.Hubble[0]  : {self.Hubble[0]:.6e}  (valore a z=0)")
        print(f"const.c         : {const.c:.6e}")
        print(f"Hubble/c [z=0]  : {self.Hubble[0] / const.c:.6e}")
        print()
        ...
        print(f"H0/c atteso [1/Mpc]  (se H in km/s/Mpc) : {H0_km / c_km:.6e}")
        print(f"H0   atteso [1/Mpc]  (se H già in 1/Mpc): {H0_km / 100 * 3.336e-4:.6e}")
        print()
        print(f"ratio (Hubble/c) / (H0_km/c_km) : {(self.Hubble[0] / const.c) / (H0_km / c_km):.6f}")"""
        ...
        for galaxy_bin in xrange(n_bins):
            ...
            #                                                                         1/Gpc * [Mpc]* [1/Mpc] *[Mpc]
            self.window_function[name].append(si.interp1d(self.z_integration,tmp_interp(self.z_integration) * (jac_dl) * ((self.Hubble / const.c) * h) / norm_const[galaxy_bin] * bias[galaxy_bin], 'cubic',bounds_error=False, fill_value=0.))
    ...
    # -----------------------------------------------------------------------------------------
    def load_galaxy_clustering_window_functions_old(self,H0, z, nz, ll, bias=1.0, name='galaxy'):
        """
        This function computes the window function for galaxy clustering given a galaxy distribution.
        The function automatically normalizes the galaxy distribution such that the integral over
        redshifts is 1.
        ...
        for galaxy_bin in xrange(n_bins):
            ...
            #                                                                      [...]                    *    [Mpc/h]->[Mpc]
            self.window_function[name].append(si.interp1d(self.z_integration,tmp_interp(self.z_integration) * ((self.Hubble / const.c)*h) * 1 /norm_const[galaxy_bin] * bias[galaxy_bin], 'cubic',bounds_error=False, fill_value=0.))
    ...
    def load_gw_lensing_window_functions_old(self, z, ndl, H_0, omega_m, omega_b, ll, name='lensing_GW'):
        ...
        for galaxy_bin in xrange(n_bins):
            ...
            self.window_function[name].append(
                si.interp1d(self.z_integration, constant * n_z_interp(self.z_integration) / norm_const[galaxy_bin],
    ...
    def load_galaxy_lensing_window_functions_old(self, z, nz, H_0, omega_m, ll, name='galaxy_lensing'):
        constant = 3. * omega_m * (H_0 / const.c) ** 2.
        nz = np.array(nz)
        z = np.array(z)
        n_bins = len(nz)
        ...
        for galaxy_bin in xrange(n_bins):
            ...
            def Wp1(ll):
                return Lp1(ll) * nz_interp(((2 * ll + 1 + 4) / (2 * ll + 1)) * self.z_integration) * f_interp(
                    ((2 * ll + 1 + 4) / (2 * ll + 1)) * self.z_integration)
            #self.Hubble / const.c è in h/Mpc
            self.window_function[name].append([si.interp1d(self.z_integration,
                                                           (Wm1(l) + Wz(l) + Wp1(l)) * ((self.Hubble / const.c)*0.6781) /
    ...
    def load_lsd_window_functions_old(self, z, ndl, H_0, omega_m, omega_b, ll, name='LSD'):
        ...
        print(conversion.H(z).value) #km/s/Mpc
        print(conversion.comoving_distance(z).value) # Mpc

        conf_H = (conversion.H(z).value / (1 + z))/const.c # km/s/Mpc -> 1/Mpc
        r_conf_H = conversion.comoving_distance(z).value * conf_H # km/s -> Adim
        gamma = r_conf_H / (1 + r_conf_H)

        jj = ((1 + z) * const.c / conversion.H(z).value + (conversion.luminosity_distance(z).value) / (1 + z)) # Mpc
        ...
        for galaxy_bin in xrange(n_bins):
            ...
            def Wp1(ll):
                return Lp1(ll) * nz_interp(((2 * ll + 1 + 4) / (2 * ll + 1)) * self.z_integration) * f_interp(
                    ((2 * ll + 1 + 4) / (2 * ll + 1)) * self.z_integration)
            # self.Hubble / const.c is in [h/Mpc]
            self.window_function[name].append([si.interp1d(self.z_integration,
                                                           (Wm1(l) + Wz(l) + Wp1(l)) * ((self.Hubble / const.c)*0.6781) /
                                                           norm_const[galaxy_bin], 'cubic', bounds_error=False,
                                                           fill_value=0.) for l in ll])
    ...
    def limber_angular_power_spectra_old(self, l, windows=None):
        ...
        n_l = len(np.atleast_1d(l))
        n_z = self.n_z_integration
        #HH = self.Hubble # self.Hubble km/s/Mpc
        cH_chi2 = self.c_over_H_over_chi_squared # [h^2/Mpc]
        ...
        for il in xrange(n_l):
            for iz in range(n_z):
                k_h = ((l[il] + 0.5) / self.geometric_factor[iz])  # k in h/Mpc
                PS_lz[il, iz] = np.asarray(power_spectra([(zz[iz], k_h)])).item()  # PS_lz in (Mpc/h)^3
        ...
        """np.save('debug_old_cH_chi2.npy', cH_chi2)  # shape (n_z,)
        np.save('debug_old_PS_lz.npy', PS_lz)  # shape (n_l, n_z)

        # W viene costruito dentro il loop per ogni key — salvalo prima
        for index_X in xrange(nkeys):
            key_X = list(keys)[index_X]
            W_X = np.array([[windows_to_use[key_X][i, j](zz) for j in range(n_l)] for i in range(n_bins[index_X])])
            np.save(f'debug_old_W_{key_X}.npy', W_X)
            print(f"  W_old[{key_X}] shape : {W_X.shape}")

        print("Salvati: cH_chi2, PS_lz, W per ogni key (old)")
        print(f"  cH_chi2 shape : {cH_chi2.shape}")
        print(f"  PS_lz shape   : {PS_lz.shape}")"""
                    ...
                    for bin_i in xrange(n_bins[index_X]):
                        for bin_j in xrange(bin_i, n_bins[index_Y]):
                            Cl['%s-%s' % (key_X, key_Y)][bin_i, bin_j] = \
                                [simpson((cH_chi2) * W_X[bin_i, xx] * W_Y[bin_j, xx] * PS_lz[xx], x=zz) for xx in
                                range(n_l)]
                            Cl['%s-%s' % (key_X, key_Y)][bin_j, bin_i] = Cl['%s-%s' % (key_X, key_Y)][bin_i, bin_j]
                # Symmetry C_{AB}^{ij} == C_{BA}^{ji}
                else:
    ...
    def limber_angular_power_spectra_lensing_auto_old(self, l, s_gal, beta, H_0, omega_m, omega_b, windows=None,
                                                  n_points=20, n_points_x=20, grid_x='mix', n_low=5, n_high=5,
                                                  Delta_z=0.05, z_min=1e-05):
        ...
        def JJ(tracer, x):
            if 'gal' in tracer:
                ...
                return (1 + x) * const.c / H(x) + (cosmo.comoving_distance(x).value) # Mpc
        ...
        def lensing_int(z1, z2, ll, bin_i, bin_j, WX_, WY_, tX, tY,
            ...
            t1 = np.transpose(A_L(rx, x, tX, r1, b1(x), Hx) * A_L(rx, x, tY, r2, b2(x), Hx) * (
                        1 + x) ** 2 / Hx * rx ** 2, (1, 2, 0))
            ...
            PS_ = np.squeeze(np.asarray([[[[power_spectra((z_val, k_val)) for k_val, z_val in
                                            zip((ll[l] + 0.5) / rx[:, k, i], x[:, k, i])] for i in range(len(z1))] for
                                          k in range(len(z2))] for l in range(len(ll))]))

            my_int = t1[None, :, :, :,] * PS_

            I1_ = np.asarray([[[trapezoid(my_int[l, i2, i1], x=x[:, i2, i1], axis=0) for i1 in range(len(z1))] for i2
            ...
            I2_ = np.asarray([[trapezoid(WY_[:, i1] * JJ(tY, z2[:, i1]) * I1_[l, :, i1] / r2[:, i1], x=z2[:, i1],
                                        axis=0) for i1 in range(len(z1))] for l in range(len(ll))])

            I3_ = trapezoid(WX_ * JJ(tX, z1) * I2_ / r1, x=z1, axis=1)
        ...
        for index_X in xrange(nkeys):
            ...
            for i, a in enumerate(zzs):
                ...
                if not len(a) == l_:
                    n_ = l_ - len(a)
                    # print('adding %s'%n_)
                    zzs[i] = np.sort(np.unique(
                        np.concatenate([zzs[i], np.linspace(z_min * (1 + 0.01), max(a) * (1 - 0.01), n_)])))
                    # print('new len %s: %s'%(i, len(zzs[i])))
            try:
                zzs = np.asarray(zzs)
            except Exception as e:
            ...
            # now compute window functions
            W_X = np.array(
                [[windows_to_use[key_X][i, j](zzs[i]) for j in range(n_l)] for i in range(n_bins[index_X])])
                    ...
                    for bin_j in myrange:
                        if (
                            ...
                            Cl['%s-%s' % (key_X, key_Y)][bin_i, bin_j, :] = l ** 2 * (l + 1) ** 2 / (
                                        (l + 0.5) ** 4) * lensing_int(zzs[bin_i], z2s_, l, bin_i, bin_j, WX, WY,
                                                                      key_X, key_Y, z_min=z_min, n_points=n_points_x,
                                                                      grid=grid_x)
    ...
    def limber_angular_power_spectra_lensing_cross_old(self, l, s_gal, beta, H_0, omega_m, omega_b, windows=None,
        ...
        #print("z_interp_bias shape:", z_interp_bias.shape)  # (1200,)
        #print("s_gal shape:", np.shape(s_gal))  # deve essere (1200,)
        #print("beta shape:", np.shape(beta))

        def H(x):
            return cosmo.H(x).value/const.c #  1/Mpc
        ...
        def JJ(tracer, x):
            if 'gal' in tracer:
                ...
                return (1 + x) * 1 / H(x) + (cosmo.comoving_distance(x).value) #Mpc
        ...
        def lensing_int(z1, z2, ll, bin_i, bin_j, WX_, WY_, tX, tY,
            ...
            """PS_ = np.squeeze(np.asarray([[[[power_spectra((z_val, k_val)) for k_val, z_val in
                                            zip((ll[l] + 0.5) / r2[:, k, i], z2[:, k, i])] for i in range(len(z1))] for k in range(len(z2))] for l in range(len(ll))]))"""

            PS_ = np.squeeze(np.asarray([[[
                power_spectra((z_val, k_val)) for k_val, z_val in
                zip((ll[l] + 0.5) / r2[:, i], z2[:, i])  # ← rimosso k, r2/z2 sono 2D
            ] for i in range(len(z1))] for l in range(len(ll))]))
            # shape risultante: (n_l, Nz1, Npts)
            ...
            I1_ = np.asarray([[trapezoid(my_int[l, i1], x=z2[:, i1], axis=0) for i1 in range(len(z1))] for l in range(len(ll))])
            ...
            I2_ = trapezoid(WX_ * (I1_ / r1) * (1 / H(z1)), x=z1, axis=1)
                    ...
                    for bin_j in myrange:
                        if (bin_j == bin_i):  # and (key_Y==key_X): # and ('gal' in key_Y) and (bin_j<=2)
                            ...
                            Cl['%s-%s' % (key_X, key_Y)][bin_i, bin_j, :] = l * (l + 1) / (
                                        (l + 0.5) ** 2) * lensing_int(zzs[bin_i], z2s_, l, bin_i, bin_j, WX, WY,
                                                                      key_X, key_Y, z_min=z_min, n_points=n_points_x,
                                                                      grid=grid_x)
