...
def merger_rate_dl_test(chi,alpha_M_interp,z_max,dl, bin_dl, log_dl,log_delta_dl, A, Z_0, Alpha, Beta, normalize=True):
    ...
    def dL_from_C(chi_interp, z,alpha_interp):
        ...
        """chi = np.asarray(C.comoving_distance(z)) / C.h  # M
    ...
    def z_from_dL(chi_interp, dL_Mpc,alpha_interp, z_M=10.0, ngrid=200):
        """
        ...
        #t0 = time.time()
        dL_grid = dL_from_C(chi_interp, z_grid,alpha_interp) # [Mpc/h]
        #t1 = time.time()
    ...
    # z dai dl originali
    z_raw = z_from_dL(chi, z_M=z_max, dL_Mpc=np.asarray(dl, dtype=float),
                      alpha_interp=alpha_M_interp)

    # se lo spacing in z supera la soglia, infittisci la griglia dl
    dz_raw = np.diff(z_raw)
    if dz_raw.max() > 0.025:
        # numero di punti necessari per avere dz < 0.025 sull'intero range z
        n_fine = int(np.ceil((z_raw.max() - z_raw.min()) / 0.025)) + 1
        n_fine = max(n_fine, len(dl))  # non scendere sotto la risoluzione originale
        # griglia dl fine che copre lo stesso range dei dl originali
        dl = np.linspace(dl.min(), dl.max(), n_fine)
        z = z_from_dL(chi, z_M=z_max, dL_Mpc=np.asarray(dl, dtype=float),
                      alpha_interp=alpha_M_interp)
    else:
        z = z_raw
    ...
    #t0 = time.time()
    bin_z_converted = z_from_dL(chi,z_M=z_max, dL_Mpc= 1e3*np.asarray(bin_dl), alpha_interp=alpha_M_interp)  # bin_dl given in Mpc
    #t1 = time.time()
    ...
    return z, bin_z_converted, merger_rate, I, merger_rate_GW   # merger_rate_GW dimensionless; merger_rate= [h/Gpc]


def merger_rate_dl_new(chi,alpha_M_interp,z_max,dl, bin_dl, log_dl,log_delta_dl, A, Z_0, Alpha, Beta, normalize=True):
    ...
    #t0 = time.time()
    z = z_from_dL(chi,z_M=z_max, dL_Mpc=np.asarray(dl, dtype=float), alpha_interp=alpha_M_interp)  # dl in Mpc
    dz = np.diff(z)
    #t1 = time.time()
...
def loc_error_param_old(bin_edges_dl, log_loc, log_dl, l_min, l_max):
...
def loc_error_param(bin_edges_dl, log_loc, log_dl, l_min, l_max):
    ...
    ell_gw = np.arange(l_min, l_max + 1, 1)  # (N_ell,)
    ell_factor = ell_gw * (ell_gw + 1)  # (N_ell,)
    const = (2 * np.pi) ** (3 / 2)
    B_avg_paper = np.zeros((n_bins, l_max - l_min + 1))


    #print(bin_edges_dl.shape,log_loc.shape,log_dl.shape)
    #print(B_avg_paper.shape, l_max - l_min + 1, l_max, l_min)

    #t0=time.time()

    for i in range(n_bins):
        mask = np.logical_and(dl > bin_edges_dl[i], dl < bin_edges_dl[i + 1])
        loc_bin = loc[mask]  # (N_gw,)

        loc_50[i] = np.percentile(loc_bin, 50)
        loc_99[i] = np.percentile(loc_bin, 1)

        # (N_gw, 1) × (1, N_ell) → broadcast (N_gw, N_ell)
        exponent = -0.5 * ell_factor[np.newaxis, :] * loc_bin[:, np.newaxis] / const
        B_bin = np.exp(exponent)  # (N_gw, N_ell)

        B_avg_paper[i] = B_bin.mean(axis=0)  # (N_ell,)

    #print("Time taken to calculate B:", time.time() - t0)
    ...
    return loc_50, max_l,B_avg_paper
...
def fisher_matrix(Cl, Cl_der, ll, f_sky=0.3, reg=1e-10):
    n_par = len(Cl_der)
    n_bins = Cl.shape[0]
    ...
    for i in range(len(ll)):
        ...
        if cond > 1 / np.finfo(float).eps:
            print(f"  [fisher_matrix] Cl[:,:,{i}] (ell={ll[i]}) is singular "
                  f"(cond={cond:.2e}) — regularizing with λ={reg:.1e}")
            mat = mat + reg * np.eye(n_bins)
        Cl_inv[:, :, i] = np.linalg.inv(mat)
    ...
    # Check Fisher matrix itself
    cond_F = np.linalg.cond(fisher)
    if cond_F > 1 / np.finfo(float).eps:
        print(f"\n  [fisher_matrix] Fisher matrix is singular (cond={cond_F:.2e})")
        # Identify which parameters are degenerate
        eigvals, eigvecs = np.linalg.eigh(fisher)
        bad = np.where(np.abs(eigvals) < reg * np.max(np.abs(eigvals)))[0]
        for idx in bad:
            dominant = np.argmax(np.abs(eigvecs[:, idx]))
            print(f"    → near-zero eigenvalue {eigvals[idx]:.2e}: "
                  f"dominant parameter index {dominant}")
        print(f"  Regularizing Fisher with λ={reg:.1e}")
        fisher = fisher + reg * np.eye(n_par)
