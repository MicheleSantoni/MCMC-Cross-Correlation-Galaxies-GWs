...
def compute_k_max(z_values, P_interp, k_range, sigma_target=0.25):
    ...
    def sigma_squared(R, z_):
        ...
        return integrate.quad(lambda k: integrand(k).item(), k_range[0], k_range[-1], limit=10000)[0]

    def find_bracket(z_):
        """Trova automaticamente un bracket [a, b] dove f cambia segno."""
        R_test = np.logspace(-2, 3, 100)   # da 0.01 a 1000
        f_vals = np.array([sigma_squared(R, z_) - sigma_target for R in R_test])
        idx = np.where(np.diff(np.sign(f_vals)))[0]
        if len(idx) == 0:
            return None
        return [R_test[idx[0]], R_test[idx[0] + 1]]

    kmax_list = []
    for z_ in z_values:
        bracket = find_bracket(z_)
        if bracket is None:
            print(f"[compute_k_max] WARNING: no bracket found at z={z_:.3f}, using k_max fallback")
            kmax_list.append(k_range[-1])
            continue
...
# ----
def compute_beta(z_gal,H_interp,chi_interp,Omega_m, s_a, s_b, s_c, s_d, be_a, be_b, be_c, be_d):
    """
    ...
    """
    H_gal=H_interp(z_gal) # 1/Mpc
    # Compute conformal Hubble parameter
    conf_H = H_gal / (1 + z_gal) # 1/Mpc

    # Derivative of the Hubble parameter with respect to redshift
    H_dot = -(3/2) * (H_gal)**2 * Omega_m * (1 + z_gal)**3 # 1/Mpc
    der_conf_H = 1 / ((1 + z_gal)**2) * (H_dot + (H_gal)**2)

    # Intermediate variables
    r_conf_H = chi_interp(z_gal) * conf_H  # Adim
    s = s_a + s_b * z_gal + s_c * z_gal**2 + s_d * z_gal**3
    gamma = r_conf_H / (1 + r_conf_H) # Adim
...
# ----
def compute_beta_old(H0, Omega_m, Omega_b,z_gal, s_a, s_b, s_c, s_d, be_a, be_b, be_c, be_d):
    """
    Compute the redshift distortion parameter 'beta' for galaxy clustering.
    ...
    # Compute conformal Hubble parameter
    #print("Hubble in beta",cosmo.H(z_gal).value )
    H_gal = (cosmo.H(z_gal) / const.c).to(1/u.Mpc).value  # Result: 1/Mpc (pure float)
    conf_H = H_gal / (1 + z_gal) # 1/Mpc

    # Derivative of the Hubble parameter with respect to redshift
    H_dot = -(3/2) * (H_gal)**2 * Omega_m * (1 + z_gal)**3 # 1/Mpc
    der_conf_H = 1 / ((1 + z_gal)**2) * (H_dot + (H_gal)**2) # 1/Mpc

    # Intermediate variables
    r_conf_H = cosmo.comoving_distance(z_gal).value * conf_H # Adim
    s = s_a + s_b * z_gal + s_c * z_gal**2 + s_d * z_gal**3
