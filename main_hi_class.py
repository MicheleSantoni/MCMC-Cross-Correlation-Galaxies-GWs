...
import colibri.cosmology_hi_class as cc
...
if __name__=='__main__':
	...
	cosmo_params_hi = configs[config_names[1]].COSMO_PARAMS_HI
	...
	H0_true = cosmo_params_hi['h'] * 100  # if you want H0 in km/s/Mpc
	Omega_m_true = round(cosmo_params_hi['omega_m']/(cosmo_params_hi['h']**2),2)
	Omega_b_true = round(cosmo_params_hi['omega_b']/(cosmo_params_hi['h']**2),3)
	A_s = cosmo_params_hi['A_s']*10**(9)
	n_s = cosmo_params_hi['n_s']
	...
	z_gal = np.linspace(z_m, z_M, 4000)  # Redshift grid for galaxy distribution
	dl_GW = np.linspace(dlm, dlM, 4000)  # Luminosity distance grid for gravitational wave sources in Mpc
	...
	kk_nl_input = np.geomspace(1e-4, 10, 250)  # Logarithmically spaced k values [h/Mpc]
	...
	universe_hi = cc.cosmo(**cosmo_params_hi)

	# Compute nonlinear matter power spectrum using HI_CLASS (self, k, z , nonlinear)
	bg, kk_nl, zz_nl, P_vals = universe_hi.hi_class_pk(kk_nl_input, zz_nl_input,True) # kk_nl in [h/Mpc]
	...
	# It is ok to use LCDM as it is just a choice of where to put the cut
	...
	if Lensing:
		...
		def Cl_func(universe, params, gw_params, dl_GW, bin_edges_dl, z_gal, ll, b_gal, b_GW, save,
			...
			kk = np.geomspace(1e-4, 10, 250)  # in [h/Mpc]
			...
			if bg == 0 and pkz == 0:
				...
				h = params['h']
				H_0 = h * 100  # if you want H0 in km/s/Mpc
				Omega_m = round(params['omega_m'] / (h ** 2), 2)
				Omega_b = round(params['omega_b'] / (h** 2), 3)

				z_bg = np.asarray(bg['z'])
				H_bg = np.asarray(bg["H [1/Mpc]"], dtype=float)  # 1/Mpc
				chi_bg = np.asarray(bg["comov. dist."], dtype=float)  # Mpc
				Omega_m_z_bg=np.asarray(bg["Omega_m_z"], dtype=float)  # need to be corrected
	...
	Cl_GG, Cl_GWGW, Cl_GGW = Cl_func(universe_hi, cosmo_params_hi, gw_params, dl_GW, bin_edges_dl, z_gal, ll, b_gal=bias_gal,b_GW=bias_GW, save=True)
	...
	LH_fun.plot_SNR_xL(ll_total, snr_l,FLAGS.fout)
	LH_fun.plot_SNR_cumulative(ll_total, snr_l,FLAGS.fout)
